...
The t statistic for $\hat{b}_{exper}$ is $t_{exper} = .0041/.0017 \sim 2.41$.  $\hat{b}_{exper}$ is statistically significant even at the 1% level.

Sdding three more years increases log(wage) by 3(.0041) = .0123, so wage is only about 1.2% higher
...
Holding profmarg fixed, $\Delta$rdintens  = .321 $\Delta$log(sales) = (.321/100)[100 * $\Delta$log(sales)] $\sim$   .00321(%$\Delta$sales).  Therefore, if %$\Delta$sales = 10,  $\Delta$rdintens $\sim$ .032, or only about 3/100 of a percentage point.  For such a large percentage increase in sales, this seems like a practically small effect.
